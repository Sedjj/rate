...
const {getStatistic} = require('../../storage/tableTennis');
...
const inputTableTennis = config.path.storage.tableTennis.inputName || 'Reports-tableTennis-default.xlsx';
const pathInputTableTennis = path.join(storagePath, exportTemplatesDirectory, inputTableTennis);
 ...
 */
function getStatisticsTableTennis(days = 2) {
	...
	return getStatistic(query)
		...
		.then((props) => {
			try {
				return readFile(pathInputTableTennis)
					.then(file => {
