...
const {TennisModel} = require('../../models/tennis');
...
function deleteStatistic(param) {
	return TennisModel.findOneAndRemove({matchId: param.matchId, strategy: param.strategy})
		.exec()
		.catch(error => {
...
function newStatistic(param) {
	return TennisModel.find({matchId: param.matchId, strategy: param.strategy})
		.exec()
		.then(statistics => {
			if (statistics.length) {
				return Promise.resolve(null);
			}
			const statistic = new TennisModel(param);
...
function setStatistic(param) {
	return TennisModel.findOne({matchId: param.matchId, strategy: param.strategy})
		.read('secondary')
		.exec()
		.then(statistic => {
